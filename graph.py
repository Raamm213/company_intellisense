...
async def gemini_judge_node(state: AgentState):
    ...
    resolved_fields = await llm_judge_resolve(
        fields_to_judge, state["company_name"], state["raw_results"], merged, source_map, rate_limiter
    )
    ...
    final_output = {
        ...
        "judge_metadata": {
            "conflict_fields": [f for f in fields_to_judge if source_map.get(f) != "none"],
            "missing_fields": [f for f in fields_to_judge if source_map.get(f) == "none"],
            "llm_judged_fields": [f for f in resolved_fields if f in fields_to_judge and source_map.get(f) != "none"],
            "llm_filled_fields": [f for f in resolved_fields if f in fields_to_judge and source_map.get(f) == "none"],
        },
    }
