...
def _pick_best_value(
    field_name: str, values: dict[str, str | None]
) -> tuple[str | None, str]:
    """
    ...
    non_null = {
        src: val for src, val in values.items() if val is not None and str(val).strip()
    }
...
async def llm_judge_resolve(
    ...
    for i in range(0, len(fields_to_judge), batch_size):
        ...
        batch_context = {}
        for field in batch:
            batch_context[field] = {src: results[src].get(field) for src in results}
...
"""
        ...
        try:
            ...
            llm = get_gemini_llm()
            print(
                f"  [JUDGE]  Agent 2 (Judge): Processing fields {i+1} to {min(i+batch_size, len(fields_to_judge))} of {len(fields_to_judge)}..."
            )
...
async def run_judge(
    company_name: str, results: dict[str, dict], rate_limiter: RateLimiter
) -> JudgeOutput:
    """
