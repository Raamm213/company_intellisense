...
MANDATORY_FIELDS = ["Company Name", "Category", "Year of Incorporation", "Logo", "Company Headquarters"]
...
OPTIONAL_FIELDS = ["Annual Revenues", "Employee Size", "Focus Sectors / Industries", "Year-over-Year Growth Rate"]
...
def tc_completeprofile_03_validator(dataset: Dataset):
    ...
    for field in MANDATORY_FIELDS:
        if not any(param.name == field and param.value not in (None, "", " ") for param in dataset.parameters):
            failures.append(f"Missing mandatory field '{field}' — Violates business rules")
    return failures
...
def tc_completeprofile_07_validator(dataset: Dataset):
    ...
    if richness_score < len(OPTIONAL_FIELDS):
        failures.append(f"Richness score below Fortune 500 benchmark ({richness_score}/{len(OPTIONAL_FIELDS)})")
    return failures
...
ALL_COMPLETE_PROFILE_TESTS = [
    MasterTestCase("TC-COMPLETEPROFILE-01", Applicability.DATASET, tc_completeprofile_01_validator),
    MasterTestCase("TC-COMPLETEPROFILE-02", Applicability.DATASET, tc_completeprofile_02_validator),
    MasterTestCase("TC-COMPLETEPROFILE-03", Applicability.DATASET, tc_completeprofile_03_validator),
    MasterTestCase("TC-COMPLETEPROFILE-04", Applicability.DATASET, tc_completeprofile_04_validator),
    MasterTestCase("TC-COMPLETEPROFILE-05", Applicability.DATASET, tc_completeprofile_05_validator),
    MasterTestCase("TC-COMPLETEPROFILE-06", Applicability.DATASET, tc_completeprofile_06_validator),
    MasterTestCase("TC-COMPLETEPROFILE-07", Applicability.DATASET, tc_completeprofile_07_validator),
]
