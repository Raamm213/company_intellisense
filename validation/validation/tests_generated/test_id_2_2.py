...
def tc_partialprofile_02_validator(dataset: Dataset):
    ...
    if populated_count < threshold_lower:
        return (
            []
        )  # Accept profile; sparse optional fields are a warning only, not a failure
...
def tc_partialprofile_07_validator(dataset: Dataset):
    ...
    flagged = []
    for param in dataset.parameters:
        if isinstance(param.value, str) and any(
            sub in param.value.lower() for sub in ["lorem", "fake", "xxx"]
        ):
            flagged.append(
                f"Field '{param.name}' contains unverified or hallucinated data: '{param.value}'"
            )
    return flagged
...
ALL_PARTIAL_PROFILE_TESTS = [
    MasterTestCase(
        "TC-PARTIALPROFILE-01", Applicability.DATASET, tc_partialprofile_01_validator
    ),
    MasterTestCase(
        "TC-PARTIALPROFILE-02", Applicability.DATASET, tc_partialprofile_02_validator
    ),
    MasterTestCase(
        "TC-PARTIALPROFILE-03", Applicability.DATASET, tc_partialprofile_03_validator
    ),
    MasterTestCase(
        "TC-PARTIALPROFILE-04", Applicability.DATASET, tc_partialprofile_04_validator
    ),
    MasterTestCase(
        "TC-PARTIALPROFILE-05", Applicability.DATASET, tc_partialprofile_05_validator
    ),
    MasterTestCase(
        "TC-PARTIALPROFILE-06", Applicability.DATASET, tc_partialprofile_06_validator
    ),
    MasterTestCase(
        "TC-PARTIALPROFILE-07", Applicability.DATASET, tc_partialprofile_07_validator
    ),
    MasterTestCase(
        "TC-PARTIALPROFILE-08", Applicability.DATASET, tc_partialprofile_08_validator
    ),
]
