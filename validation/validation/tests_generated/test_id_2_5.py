...
class FieldRelationshipValidator:
    ...
    def validate(self, record: Dict[str, any], target: str, related: List[str]) -> List[str]:
        errors = []
        ...
        if any_related_populated and not target_populated:
            errors.append(
                f"{target} should be populated because related fields exist."
            )

        if all_related_null and target_populated:
            errors.append(
                f"{target} must be NULL because related fields are NULL."
            )
...
RELATIONSHIP_RULES = [
    ...
    ("Overview of the Company", ["Services / Offerings / Products", "Vision", "Mission", "Core Value Proposition"], "2.5.6"),
    ...
    ("Top Customers by Client Segments", ["Category", "Services / Offerings / Products"], "2.5.19"),
    ("Core Value Proposition", ["Pain Points Being Addressed", "Services / Offerings / Products"], "2.5.20"),
    ...
    ("Social Media Followers – Combined", ["Instagram Page URL", "LinkedIn Profile URL", "Facebook Page URL", "Twitter (X) Handle"], "2.5.36"),
    ("CEO LinkedIn URL", ["CEO Name"], "2.5.45"),
    ("Profitability Status", ["Annual Profits"], "2.5.65"),
    ("Market Share (%)", ["Annual Revenues", "Total Addressable Market (TAM)"], "2.5.66"),
    ("Key Investors / Backers", ["Recent Funding Rounds"], "2.5.67"),
    ("Total Capital Raised", ["Recent Funding Rounds"], "2.5.69"),
    ("CAC:LTV Ratio", ["Customer Acquisition Cost (CAC)", "Customer Lifetime Value (CLV)"], "2.5.74"),
    ("Customer Concentration Risk", ["Revenue Mix", "Top Customers by Client Segments"], "2.5.77"),
    ("Runway", ["Total Capital Raised", "Burn Rate"], "2.5.79"),
    ("Burn Multiplier", ["Burn Rate", "Annual Revenues"], "2.5.80"),
    ("Serviceable Addressable Market (SAM)", ["Total Addressable Market (TAM)"], "2.5.109"),
    ("Serviceable Obtainable Market (SOM)", ["Serviceable Addressable Market (SAM)"], "2.5.110"),
    ("Central vs peripheral location", ["Company Headquarters", "Office Locations"], "2.5.123"),
    ("Commute time from airport", ["Company Headquarters", "Office Locations"], "2.5.126"),
    ("Office zone type", ["Company Headquarters", "Office Locations"], "2.5.127"),
    ("Area safety", ["Company Headquarters", "Office Locations"], "2.5.128"),
    ("Company maturity", ["Year of Incorporation", "Employee Size", "Annual Revenues"], "2.5.146"),
...
def test_related_present_requires_target(target, related, test_id, validator):
    ...
    assert errors, f"{test_id} FAILED: {target} not populated despite related fields present."
