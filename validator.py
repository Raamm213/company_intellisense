...
class CompanyValidator:
    def __init__(self, metadata_path: str = None):
        ...
        if metadata_path is None:
            metadata_path = base_dir / "validation" / "validation" / "metadata params.csv"
        ...
        mapping_path = base_dir / "validation" / "validation" / "tests_generated" / "metadata_mapping.csv"
        if not mapping_path.exists():
            mapping_path = Path("/app/validation/validation/tests_generated/metadata_mapping.csv")
    ...
    def load_rules(self):
        ...
        if not os.path.exists(self.metadata_path):
            print(f"[Validator Warning] Metadata file not found at {self.metadata_path}")
            return
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                self.rules[col_name] = {
                    'regex': str(row['regex_pattern']) if pd.notna(row['regex_pattern']) else None,
                    'nullability': str(row['nullability']).strip().lower(),
                    'min_len': row['minimum_element'] if pd.notna(row['minimum_element']) else None,
                    'max_len': row['maximum_element'] if pd.notna(row['maximum_element']) else None,
                    'category': row['category']
                }
            print(f"[Validator] Loaded {len(self.rules)} rules from {self.metadata_path}")
        except Exception as e:
    ...
    def validate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_rule_name, rule in self.rules.items():
            ...
            if not rule_failed and not is_empty:
                ...
                if reg_pattern and isinstance(reg_pattern, str) and reg_pattern != "None" and reg_pattern.strip():
                    try:
                        ...
                        if not re.search(pattern, str(value).strip()):
                            errors.append(f"Field '{field_rule_name}' format mismatch ({value}).")
                            rule_failed = True
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        if min_l is not None and min_l != "None":
                            if len(val_str) < int(float(str(min_l))):
                                errors.append(f"Field '{field_rule_name}' is too short.")
                                rule_failed = True
